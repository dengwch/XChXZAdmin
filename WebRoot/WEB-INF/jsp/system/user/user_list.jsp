			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						-->
						<th><i class="icon-time hidden-phone"></i>开通时间</th>
						<th>状态</th>
							...
							<tr>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit == 1 }">
											<!-- <c:if test="${user.USERNAME != 'admin'}"><a class='btn btn-mini btn-info' title="重置密码" onclick="chongzhi('${user.USER_ID }');"><i class='icon-edit'></i></a></c:if>
											-->
											<c:if test="${user.STATUS == '0' }">
												<c:if test="${user.USERNAME != 'admin'}"><a class='btn btn-mini btn-info' title="停用" onclick="tingyong('${user.USER_ID }','1');"><i class='icon-edit'></i></a></c:if>
											</c:if>
											<c:if test="${user.STATUS == '1' }">
												<c:if test="${user.USERNAME != 'admin'}"><a class='btn btn-mini btn-info' title="激活" onclick="huifu('${user.USER_ID }','0');"><i class='icon-edit'></i></a></c:if>
											</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		<script type="text/javascript">
			function chongzhi(USER_ID){
				bootbox.confirm("你确定要将密码重置为123456吗？", function(result) {
					if(result) {
						top.jzts();
						var PASSWORD="123456";
						window.location.href="<%=basePath%>user/listUsers?USER_ID="+USER_ID+"&PASSWORD="+PASSWORD;
					}
				});
			}
			function tingyong(USER_ID,STATUS){
				bootbox.confirm("你确定要将此号停用吗", function(result) {
					if(result) {
						top.jzts();
						window.location.href="<%=basePath%>user/listUsers?USER_ID="+USER_ID+"&STATUS="+STATUS;
					}
				});
			}
			function huifu(USER_ID,STATUS){
				bootbox.confirm("你确定要将此号恢复吗", function(result) {
					if(result) {
						top.jzts();
						window.location.href="<%=basePath%>user/listUsers?USER_ID="+USER_ID+"&STATUS="+STATUS;
					}
