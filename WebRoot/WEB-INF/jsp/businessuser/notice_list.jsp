...
<form action="businessuser/Noticelist.do" method="post" name="Form" id="Form">
<table>
				...
				<c:if test="${QX.cha == 1 }">
					<c:forEach items="${nameList}" var="var" varStatus="vs">
						<tr>
							...
							<td class='center' style="vertical-align: middle;">${var.CONTENT}</td>
							...
							<c:if test="${var.STATUS==2}">
								<td class='center' style="vertical-align: middle;">未处理</td>
							</c:if>
							...
							<c:if test="${QX.edit != 1 && QX.del != 1 }">
								...
								<c:if test="${var.STATUS==2}">
								<c:if test="${var.ZCFLAG==1}">
									<a class="btn btn-small btn-danger"
										onclick="gFlag('${var.BUSINESSUSER_ID}','2');">审核通过</a>
									<a class="btn btn-small btn-danger"
										onclick="gFlag('${var.BUSINESSUSER_ID}','3');">审核失败</a>
								</c:if>
								<c:if test="${var.ZCFLAG==2}">
								已处理
								</c:if>
								<c:if test="${var.ZCFLAG==3}">
								已处理
								</c:if>
								<c:if test="${var.ZCFLAG==4}">
								未填写审核信息不可处理
								</c:if>
								
								
									<!--<a class="btn btn-small btn-danger"
										onclick="gFlag('${var.BUSINESSUSER_ID}','2');">确认处理</a>
									--><!--<a class="btn btn-small btn-danger"
										onclick="gFlag('${var.BUSINESSUSER_ID}','3');">审核失败</a>
								-->
								</c:if>
...
<script type="text/javascript">
		...
		function gFlag(Id,flag){
			var url = "<%=basePath%>businessuser/Noticelist.do?BUSINESSUSER_ID="+Id+"&ZCFLAG2="+flag;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>businessuser/goEdit1.do?BUSINESSUSER_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function editpass(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>businessuser/goPassword.do?BUSINESSUSER_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
