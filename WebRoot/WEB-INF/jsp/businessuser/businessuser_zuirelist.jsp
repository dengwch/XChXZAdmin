...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="businessuser/listzuire.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;vertical-align: middle;">${vs.index+1}</td>
										<td class='center' style="vertical-align: middle;">${var.BUSINESSUSERNAME}</td>
										<td class='center' style="vertical-align: middle;">${var.SHOPNAME}</td>
								<td style="width: 260px;vertical-align: middle;" class="center" >
										
										<c:if test="${QX.edit == 1}">
											<a class="btn btn-small btn-danger" onclick="del('${var.BUSINESSUSER_ID}');" >删除</a>
										</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>businessuser/goAddzui.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>businessuser/delete1.do?BUSINESSUSER_ID="+Id+"&tm="+new Date().getTime();
