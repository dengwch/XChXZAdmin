...
<body>
	<form action="agentmanger/${msg }.do" name="Form" id="Form" method="post">
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商用户名:</td>
				<td><input type="text" name="USERNAME" id="USERNAME" value="${pd.USERNAME}" maxlength="32" placeholder="这里输入代理商用户名" title="代理商用户名"/></td>
			</tr>
			 <tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商登录密码:</td>
				<td><input type="text" name="PWD" id="PWD" value="" maxlength="32" placeholder="这里输入代理商登录密码" title="代理商登录密码"/></td>
			</tr> 
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商名:</td>
			...
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商区域:</td>
				<td>
			...
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">联系人:</td>
				<td><input type="text" name="CONTACTS" id="CONTACTS" value="${pd.CONTACTS}" maxlength="32" placeholder="这里输入联系人" title="联系人"/></td>
			</tr>
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">联系电话:</td>
			...
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商负责人身份证:</td>
				<td><input type="text" name="CARDNO" id="CARDNO" value="${pd.CARDNO}" maxlength="32" placeholder="这里输入代理商负责人身份证" title="代理商负责人身份证"/></td>
			</tr>
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商负责人身份证正面照片:</td>
			...
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商负责人身份证反面照片:</td>
			...
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商公司名:</td>
			...
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">组织机构代码:</td>
				<td><input type="text" name="COMPANYNO" id="COMPANYNO" value="${pd.COMPANYNO}" maxlength="32" placeholder="这里输入组织机构代码" title="组织机构代码"/></td>
			</tr>
			<tr>
				<td style="width:150px;text-align: right;padding-top: 13px;">代理商营业执照:</td>
				<td>
			...
			<tr>
				...
				<!-- 开始循环 -->	
						...
						<c:forEach items="${jiaoList}" var="var1" varStatus="vs">
							...
							-->
							<c:if test="${var1.SALE!=null && var1.SALE!='' && var1.SALE!=undefined}">
								<!--<option value="${var1.SALE1 }">${var1.SALE }</option>
								<c:if test="${var1.SALE!='1折' }">
									<option value="0.1">1折</option>
								</c:if>
								<c:if test="${var1.SALE!='2折' }">
									<option value="0.2">2折</option>
								</c:if>
								<c:if test="${var1.SALE!='3折' }">
									<option value="0.3">3折</option>
								</c:if>
								<c:if test="${var1.SALE!='4折' }">
									<option value="0.4">4折</option>
								</c:if>
								<c:if test="${var1.SALE!='5折' }">
									<option value="0.5">5折</option>
								</c:if>
								<c:if test="${var1.SALE!='6折' }">
									<option value="0.6">6折</option>
								</c:if>
								<c:if test="${var1.SALE!='7折' }">
									<option value="0.7">7折</option>
								</c:if>
								<c:if test="${var1.SALE!='8折' }">
									<option value="0.8">8折</option>
								</c:if>
								<c:if test="${var1.SALE!='9折' }">
									<option value="0.9">9折</option>
								</c:if>
							-->
							<input onblur="tive('${var1.BUSINESS_ID}');" type="text" name="SAL${var1.BUSINESS_ID}" id="SAL${var1.BUSINESS_ID}" value="${var1.SALE }" maxlength="32" placeholder="这里输入折扣" title="折扣"/>
							</c:if>
							<c:if test="${var1.SALE==null || var1.SALE=='' || var1.SALE==undefined}">
								 <!--<option value="选择">选择</option>
										<option value="0.1">1折</option>
							...
							<input onblur="tive('${var1.BUSINESS_ID}');" type="text" name="SAL${var1.BUSINESS_ID}" id="SAL${var1.BUSINESS_ID}" value="" maxlength="32" placeholder="这里输入折扣" title="折扣"/>		
								...
								<td class="center" id="qe${var1.BUSINESS_ID}">
									<a class="btn btn-small btn-success" onclick="delbus('${var1.BUSINESS_ID}');">取消添加</a>
								</td>
							</tr>
			...
			<tr>
				<td style="text-align: center;" colspan="10">
					...
					<a class="btn btn-mini btn-danger" onclick="closeX('${pd.AGENTMANGER_ID}');">取消</a>
				</td>
			</tr>
		</table>
		</div>
		...
		function addbus(id){
			...
			$("#SALE").val(SALE);
			$("#qe"+id).empty();
			$("#qe"+id).append("<a class='btn btn-small btn-success' onclick='delbus(\""+id+"\");'>取消添加</a>");
		}
		//取消添加折扣
		function delbus(id){
			var r="";
			...
			$("#SALE").val(r);
			$("#qe"+id).empty();
			$("#qe"+id).append("<a class='btn btn-small btn-success' onclick='addbus(\""+id+"\");'>添加</a>");
