...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="transactionrecord/listCZ.do" method="post" name="Form" id="Form">
			...
			<table> 
				<tr>
					...
					<td>
						<button style="margin-bottom:10px;" class="btn btn-mini btn-light" onclick="px('1');">按充值金额排序</button>
					</td>
					<td>
						<button style="margin-bottom:10px;" class="btn btn-mini btn-light" onclick="px('2');">按购买日期排序</button>
					</td>
					
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="PAYTYPE" id="PAYTYPE" data-placeholder="请选择充值方式" style="vertical-align:top;width: 120px;">
						<option value=""></option>
						<option value="">全部</option>
						<option value="0" <c:if test="${pd.PAYTYPE=='0'}">selected</c:if>>支付宝</option>
						<option value="1" <c:if test="${pd.PAYTYPE=='1'}">selected</c:if>>微信</option>
					  	</select>
					</td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<c:if test="${var.PAYTYPE=='0'}">
											<td class="center">支付宝</td>
										</c:if>
										<c:if test="${var.PAYTYPE=='1'}">
